...
class TestExtractHotWords:
    def test_camel_case_extraction(self, tmp_path):
        db = _make_db(tmp_path, [
            {"payload": {"ocr_text": "Working on ContextPulse and PhotoEditor today", "ocr_confidence": 0.9}},
        ])
        words = extract_hot_words(db_path=db, seconds=60)
        assert "ContextPulse" in words
        assert "PhotoEditor" in words
    ...
    def test_frequency_sorting(self, tmp_path):
        db = _make_db(tmp_path, [
            {"payload": {"ocr_text": "ContextPulse ContextPulse PhotoEditor", "ocr_confidence": 0.9}},
        ])
        words = extract_hot_words(db_path=db, seconds=60)
    ...
    def test_min_confidence_filtering(self, tmp_path):
        db = _make_db(tmp_path, [
            ...
            {"payload": {"ocr_text": "HighConfidence PhotoEditor", "ocr_confidence": 0.9}},
        ])
...
class TestBuildWhisperPrompt:
    def test_basic_prompt(self):
        prompt = build_whisper_prompt(["ContextPulse", "PhotoEditor"])
        assert "ContextPulse" in prompt
        assert "PhotoEditor" in prompt

    def test_combines_hot_words_and_nouns(self):
        prompt = build_whisper_prompt(
            ["ContextPulse"], ["WeatherApp", "DataPipeline"],
        )
        assert "ContextPulse" in prompt
        assert "WeatherApp" in prompt
        assert "DataPipeline" in prompt

    def test_deduplication_preserves_hot_word_order(self):
        prompt = build_whisper_prompt(
            ["ContextPulse", "PhotoEditor"],
            ["ContextPulse", "WeatherApp"],
        )
        # ContextPulse should appear once, and hot-word version comes first
        assert prompt.count("ContextPulse") == 1
        assert prompt.index("ContextPulse") < prompt.index("WeatherApp")
