...
class TestSplitCamelToPhrase:
    ...
    def test_multi_part(self):
        assert _split_camel_to_phrase("DataPipeline") == "data pipeline"
    ...
    def test_music_player(self):
        assert _split_camel_to_phrase("MusicPlayer") == "music player"

    def test_code_review(self):
        result = _split_camel_to_phrase("CodeReviewBot")
...
class TestExtractNamesFromContext:
    ...
    def test_finds_quoted_names(self):
        text = 'The product is called "MusicPlayer" and it streams audio.'
        names = _extract_names_from_context(text)
        assert "MusicPlayer" in names
...
class TestBuildContextVocabulary:
    ...
    def test_skips_common_phrases(self, tmp_path):
        (tmp_path / "StockMarket").mkdir()
        vocab = build_context_vocabulary(tmp_path, skills_dirs=[])
        # "stock market" is in _COMMON_PHRASES
        assert "stock market" not in vocab
